...
class GCN(torch.nn.Module):
    def __init__(self, input_dim, args, num_nodes):
        ...
        mp_layer_dim=90-args.delete_feature
        num_classes=args.num_classes

        for i in range(num_layers - 1):
            conv = torch_geometric.nn.Sequential('x, edge_index, edge_attr', [
                (MPGCNConv(gcn_input_dim, hidden_dim, edge_emb_dim, gcn_mp_type, bucket_sz, normalize=True, bias=True),
        ...
        if self.pooling == "concat":
            node_dim = 8
            conv = torch_geometric.nn.Sequential('x, edge_index, edge_attr', [
                (MPGCNConv(hidden_dim, hidden_dim, edge_emb_dim, gcn_mp_type, bucket_sz, normalize=True, bias=True),
                 'x, edge_index, edge_attr -> x'),
            ...
            if num_layers == 1:
                conv = torch_geometric.nn.Sequential('x, edge_index, edge_attr', [
                    (MPGCNConv(mp_layer_dim, hidden_dim, edge_emb_dim, gcn_mp_type, bucket_sz, normalize=True, bias=True),
                     'x, edge_index, edge_attr -> x'),
                    nn.Linear(hidden_dim, hidden_dim),
                    nn.LeakyReLU(negative_slope=0.2),
                    nn.BatchNorm1d(node_dim)
        ...
        self.pool=MLP(input_dim=args.hidden_dim,hidden_dim=args.hidden_dim,num_layers=1,activation=torch.nn.ReLU,n_classes=num_classes)
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        if self.pooling == "concat":
            z = h.reshape((h.shape[0] // self.num_nodes, -1))
        elif self.pooling == 'sum':
            z = global_add_pool(h, batch)  # [N, F]
        elif self.pooling == 'mean':
            z = global_mean_pool(h, batch)  # [N, F]
